...
show_quick_help() {
    ...
    echo -e "  ${GREEN}yoyo-gui${RESET}              Launch browser dashboard standalone"
    echo -e "  ${GREEN}yoyo-gui --port 8080${RESET}  Use custom port (default: 3456)"
    echo -e "  ${GREEN}yoyo-gui --dev${RESET}        Development mode"
    echo -e "  ${GREEN}yoyo-gui --stop${RESET}       Stop background GUI server"
    ...
    echo -e "  ${GREEN}yoyo${RESET}             TUI + Claude + GUI (default)"
    echo -e "  ${GREEN}yoyo --no-gui${RESET}    TUI + Claude without GUI"
