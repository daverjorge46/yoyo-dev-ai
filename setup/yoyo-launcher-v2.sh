...
show_help() {
    ...
    echo ""
    echo -e "  ${DIM}Visual mode: 3-pane layout (Main | Task Status | File Explorer)${RESET}"
    echo -e "  ${DIM}Set YOYO_VISUAL_MODE=false to disable visual mode by default${RESET}"
    echo ""
    echo -e "${BOLD}Visual Mode Panes:${RESET}"
    echo ""
    echo -e "  ${DIM}Left (50%):         Main Claude Code terminal${RESET}"
    echo -e "  ${DIM}Top-Right (25%):    Task status monitor${RESET}"
    echo -e "  ${DIM}Bottom-Right (25%): Interactive file manager${RESET}"
    echo ""
    echo -e "  ${DIM}File Manager: ranger (auto-install on first run)${RESET}"
    echo ""
    echo -e "  ${DIM}Controls:${RESET}"
    echo -e "    Ctrl+B then arrows  - Switch panes"
    echo -e "    Ctrl+B then z       - Full screen toggle"
    echo -e "    Ctrl+B then x       - Close pane"
    echo -e "    Ctrl+B then Alt+arrows - Resize panes"
    echo ""
    echo "────────────────────────────────────────────────────────────────────"
    echo ""
...
launch_claude() {
    ...
    echo -e "   ${CYAN}✨${RESET} 3-pane visual mode (tasks + file explorer)"
