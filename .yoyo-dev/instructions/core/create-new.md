...
<discovery_modes>
  ...
  <mode_c>
    ...
    <action>
      1. READ @.yoyo-dev/product/roadmap.md
...
<conditional_logic>
  IF both mission-lite.md AND tech-stack.md already in context:
    ...
    PROCEED to step 3
  ELSE:
    READ only missing files:
      - @.yoyo-dev/product/mission-lite.md (if not in context)
      - @.yoyo-dev/product/tech-stack.md (if not in context)
