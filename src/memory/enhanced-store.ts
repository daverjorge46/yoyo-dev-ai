...
/**
 * Check if enhanced columns exist on the memory_blocks table.
 */
...
export function ensureEnhancedSchema(store: MemoryStore): void {
  const db = store.db;
  if (!db || hasEnhancedColumns(store)) {
    return;
  }
...
export function saveEnhancedBlock(
  ...
  if (!store?.db) {
    ...
    return {
      ...
      version: input.relevanceScore ? 1 : 0,
      createdAt: now,
      updatedAt: now,
...
export function getEnhancedBlock(
  ...
  const selector = isMemoryBlock(typeOrId)
    ? { id: typeOrId.id }
    : scope === undefined
  ...
  const statement =
    'id' in selector
      ? db.prepare('SELECT * FROM memory_blocks WHERE id = ?')
      : db.prepare('SELECT * FROM memory_blocks WHERE type = ? AND scope = ?');

  const row = ('id' in selector
    ? statement.get(selector.id)
    : statement.get(selector.type, selector.scope)) as EnhancedMemoryBlockRow | undefined;
...
export function updateBlockEmbeddings(
  ...
  const usingId = typeof scopeOrEmbeddings !== 'string';
  const embeddings = usingId ? (scopeOrEmbeddings as number[]) : maybeEmbeddings;
  ...
  const selector = isMemoryBlock(idOrType)
    ? { id: idOrType.id, type: idOrType.type, scope: idOrType.scope }
    : usingId
      ? { id: idOrType as string }
      : { type: idOrType as MemoryBlockType, scope: scopeOrEmbeddings as MemoryScope };

  try {
    const result = usingId
      ? store.db
          .prepare(`
      ...
      : store.db
          ...
          `)
          .run(serialized, selector.type as MemoryBlockType, selector.scope as MemoryScope);

    return result.changes > 0;
  } catch (error) {
    console.error('[updateBlockEmbeddings] failed', {
      usingId,
      idOrType,
      scopeOrEmbeddings,
      maybeEmbeddings,
...
export function updateBlockRelevance(
  ...
  const usingId = typeof scopeOrScore !== 'string';
  const score = usingId ? (scopeOrScore as number) : maybeScore;
  ...
  const selector = isMemoryBlock(idOrType)
    ...
    : usingId
      ? { id: idOrType as string }
      : { type: idOrType as MemoryBlockType, scope: scopeOrScore as MemoryScope };
    ...
    const result = usingId
      ...
      : store.db
          .prepare(`
            ...
            WHERE type = ? AND scope = ?
          `)
          .run(clampedScore, selector.type as MemoryBlockType, selector.scope as MemoryScope);

    return result.changes > 0;
  } catch (error) {
    console.error('[updateBlockRelevance] failed', {
      usingId,
      idOrType,
      scopeOrScore,
      maybeScore,
...
export function updateBlockTags(
  ...
  const tags = usingId ? (scopeOrTags as string[]) : maybeTags;
  ...
  const selector = isMemoryBlock(idOrType)
    ...
    : usingId
      ? { id: idOrType as string }
      : { type: idOrType as MemoryBlockType, scope: scopeOrTags as MemoryScope };
    ...
    const result = usingId
      ? store.db
          .prepare(`
            ...
            WHERE id = ?
          `)
          .run(serializedTags, selector.id)
      : store.db
          .prepare(`
            UPDATE memory_blocks
            SET context_tags = ?, updated_at = datetime('now')
            WHERE type = ? AND scope = ?
          `)
          .run(serializedTags, selector.type as MemoryBlockType, selector.scope as MemoryScope);

    return result.changes > 0;
  } catch (error) {
    console.error('[updateBlockTags] failed', {
      usingId,
      idOrType,
      scopeOrTags,
      maybeTags,
