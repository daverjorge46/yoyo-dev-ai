...
export function parseInstruction(instruction) {
    ...
    if (blockMatch) {
        return {
            ...
            explicitBlock: blockMatch[1]?.toLowerCase(),
        };
    }
    return {
 ...
 */
export function detectTargetBlock(parsed) {
 ...
 */
export function updateMemoryFromInstruction(service, instruction, blockType) {
    const changes = [];
    switch (blockType) {
        case 'persona':
 ...
 */
function updatePersonaBlock(service, instruction, changes) {
 ...
 */
function updateProjectBlock(service, instruction, changes) {
    ...
    for (const fw of frameworks) {
        if (lower.includes(fw)) {
            ...
            if (fw === 'next')
                content.tech_stack.framework = 'Next.js';
            if (fw === 'nuxt')
                content.tech_stack.framework = 'Nuxt';
 ...
 */
function updateUserBlock(service, instruction, changes) {
 ...
 */
function updateCorrectionsBlock(service, instruction, changes) {
 ...
 */
function extractIssue(instruction) {
 ...
 */
function extractCorrection(instruction) {
 ...
 */
export async function rememberCommand(service, instruction) {
    // Validate input
    ...
    // Update memory
    const result = updateMemoryFromInstruction(service, parsed.rawInstruction, blockType);
