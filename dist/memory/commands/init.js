 ...
 */
export function scanProjectStructure(projectRoot) {
    const directories = [];
    const keyFiles = [];
    const fileTypes = {};
    let totalFiles = 0;
 ...
 */
export function detectPatterns(projectRoot) {
    const patterns = [];
 ...
 */
export function createInitialMemory(service, projectRoot, _options = {}) {
    ...
    if (techStack.database)
        techStackContent.database = techStack.database;
    if (techStack.styling)
        techStackContent.styling = techStack.styling;
    if (techStack.testing)
        techStackContent.testing = techStack.testing;
    if (techStack.buildTool)
        techStackContent.build_tool = techStack.buildTool;
    ...
    const expertiseAreas = [techStack.language];
    if (techStack.framework)
        expertiseAreas.push(techStack.framework);
    if (techStack.testing)
        expertiseAreas.push(techStack.testing);
 ...
 */
function buildKeyDirectories(structure) {
    const keyDirs = {};
    const descriptions = {
        src: 'Source code',
 ...
 * @returns Init result
 */
export async function initCommand(service, projectRoot, options = {}) {
